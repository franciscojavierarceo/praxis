...
pub(crate) fn responses_request_to_chat_request(request: &Value) -> Result<Value, TranslationError> {
    ...
    copy_field(obj, &mut chat, "top_logprobs");
...
fn response_resource(context: &ResponseContext, parts: &ResponseResourceParts<'_>) -> Value {
    json!({
        ...
        "presence_penalty": number_or_default(context.presence_penalty.as_ref(), 0.0),
        "frequency_penalty": number_or_default(context.frequency_penalty.as_ref(), 0.0),
        "top_logprobs": context.top_logprobs.unwrap_or(0)
    })
}
